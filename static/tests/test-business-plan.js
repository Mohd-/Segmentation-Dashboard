...
test('business-plan renders the approved dashboard and one auto-save approval detail shell', async function () {
  ...
  // Card 3F: 1999-2035 plus "All Years", which is a real option rather than a
  // cleared filter, so it lives in the same list as the years.
  ...
  assert.equal(host.querySelectorAll('#bpe-filter-row .lf-trigger').length, 4);
  ...
  // order. Card 3F removed the step filter that used to lead this row; the
  // assignee filter still closes it.
  assert.deepEqual(
...
// ---------------------------------------------------------------------------
// Card 3F -- the Pre-Drilling column's own BP Gate toggle
// ---------------------------------------------------------------------------
//
// It replaces the global step dropdown, whose default value silently narrowed
// the WHOLE board to Pre-Drilling wells. The toggle reaches only the column it
// sits in, and it filters from the payload already in hand -- no round trip,
// and the KPIs never move.
...
test('business-plan the BP Gate toggle narrows only the Pre-Drilling column', async function () {
  ...
  // The other columns are untouched -- the old dropdown emptied them.
