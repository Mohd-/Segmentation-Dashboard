...
import { refreshAllBoards, markBoardsStale } from './pipeline.js';
...
export function refreshAfterRecordChange(message) {
  return API.detail(Store.projectId)
    .then(function (detail) {
      ...
      // Item A: an auto-save lands here mid-typing. loadComponent preserves
      // the focused control and its typed value across its own re-render; a
      // no-op for every non-form flow (transition, rename, priority) that
      // also refreshes through here.
      Store.project = detail.project || {};
      Store.tasks = detail.tasks || [];
      Store.allFields = detail.fields || {};
      Store.leadSummary = detail.lead_summary || null;
      ...
      return Promise.resolve(loadComponent(nextTask)).then(function () {
        // Every flow through here happens with the detail shell open and the
        // boards hidden -- defer their refresh to the return-to-board click.
        markBoardsStale();
